...
$filtroFiltro = isset($_GET['filtro']) ? $_GET['filtro'] : '';
...
try {
    ...
    if ($filtroFiltro === 'administrador') {
        $whereClause .= " AND u.is_admin = 1";
    } elseif ($filtroFiltro === 'tem_cnh') {
        $whereClause .= " AND u.tem_cnh = 1";
    } elseif ($filtroFiltro === 'nao_tem_cnh') {
        $whereClause .= " AND u.tem_cnh = 0";
    } elseif ($filtroFiltro === 'cadastro_incompleto') {
        $whereClause .= " AND u.cadastro_completo = 0";
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['user_id'])) {
        ...
        if ($action === 'aprovar' || $action === 'rejeitar') {
            ...
            header("Location: usuarios.php?status={$filtroStatus}&page={$paginaAtual}" . 
    ...
    $_SESSION['notification'] = [
        'type' => 'error',
        'message' => 'Erro ao carregar usuarios: ' . $e->getMessage()
    ];
}
        ...
        <div class="max-w-7xl mx-auto px-4">
            ...
            <div class="backdrop-blur-lg bg-white/5 border subtle-border rounded-3xl p-6 mb-8 shadow-lg">
                <div class="flex flex-col md:flex-row gap-4">
                    <div class="flex-1">
                        <form action="usuarios.php" method="GET" class="flex flex-col sm:flex-row gap-4">
                            <div class="flex-1">
                            ...
                            <div class="sm:w-48">
                                ...
                                >
                                    ...
                                    <option value="administrador" <?= $filtroFiltro === 'administrador' ? 'selected' : '' ?>>Administrador</option>
                                    <option value="tem_cnh" <?= $filtroFiltro === 'tem_cnh' ? 'selected' : '' ?>>Tem CNH</option>
                                    <option value="nao_tem_cnh" <?= $filtroFiltro === 'nao_tem_cnh' ? 'selected' : '' ?>>Não tem CNH</option>
                                    <option value="cadastro_incompleto" <?= $filtroFiltro === 'cadastro_incompleto' ? 'selected' : '' ?>>Cadastro Incompleto</option>
                                </select>
                            </div>
            ...
            <div class="backdrop-blur-lg bg-white/5 border subtle-border rounded-3xl p-6 shadow-lg">
                <?php if ($nenhumUsuario): ?>
                    <div class="text-center py-8">
                        ...
                        <h3 class="text-xl font-medium text-white mb-2">Nenhum usuario encontrado</h3>
                        ...
                        <?php if ($filtroStatus || $filtroPesquisa): ?>
                            <a href="usuarios.php" class="inline-block mt-4 px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white font-medium rounded-xl transition-colors border border-indigo-400/30 shadow-md hover:shadow-lg">
                                Limpar Filtros
                            </a>
                        <?php endif; ?>
                    </div>
                        ...
                        <table class="w-full min-w-full">
                            <thead>
                                <tr class="border-b border-white/10">
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/70 uppercase tracking-wider">Telefone</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/70 uppercase tracking-wider">Data Cadastro</th>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-white/70 uppercase tracking-wider">Data Verificacao</th>
                                </tr>
                            </thead>
                            <tbody>
                                <?php foreach ($usuarios as $user): ?>
                                    <tr class="border-b border-white/5 hover:bg-white/5">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-white/80">
                                            <?= htmlspecialchars($user['telefone'] ?: 'Não informado') ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-white/80">
                                            <?= $user['data_de_entrada'] ? date('d-m-Y', strtotime($user['data_de_entrada'])) : 'Não informado' ?>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
    ...
    <script>
        ...
        function abrirModalDocumentos(userId, nomeUsuario) {
            ...
            if (userData.foto_cnh_frente) {
                imgCnhFrente.src = '../' + userData.foto_cnh_frente;
            ...
            if (userData.foto_cnh_verso) {
                imgCnhVerso.src = '../' + userData.foto_cnh_verso;
